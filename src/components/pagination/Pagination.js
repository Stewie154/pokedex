...
const Pagination = () => {
	...
	return (
		<div className="h-[10%] flex justify-around items-center">
			<button 
				className={`${pokemon_list[0] === first_pokemon_being_displayed ? 'hidden' : 'light-mode-bg-secondary text-lg font-bold px-7 py-2 border rounded-lg transition-all duration-400'}`}
				onClick={() => handlePrevBtnClick()}
			>
				Prev
			</button>
			<button 
				className={`${pokemon_list[pokemon_list.length -1] === last_pokemon_being_displayed ? 'hidden' : 'light-mode-bg-secondary text-lg font-bold px-7 py-2 border rounded-lg transition-all duration-400'}`}
